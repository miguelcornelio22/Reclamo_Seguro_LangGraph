...
🔹 Lógica del grafo
Flujo lineal: recibir_reclamo → validar_poliza → validar_cobertura → estimar_monto → tomar_decision.

Condicional en tomar_decision:

"Pendiente" → va a solicitar_aprobacion.

Cualquier otra decisión → va a generar_respuesta.

Loop controlado: solicitar_aprobacion → tomar_decision.

Finalización: generar_respuesta → END.
